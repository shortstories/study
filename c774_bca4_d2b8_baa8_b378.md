...
5. Thread 갯수가 지나치게 많아지면 문제 발생
  ...
  8. Context switching 비용
    9. 하나의 core는 한번에 한 thread만 실행 가능
    9. Thread의 상태 Run, Waiting, Ready, Sleep, Blocked 중 Waiting, Sleep, Blocked 상태인 Thread를 골라 실행하는데 이 때, thread가 가지고 있는 stack 정보를 core의 register로 복사하는 작업이 이루어지게 됨
  9. thread 경합
    10. 공통되는 자원이 있을 시 단일 접근 권한을 얻기 위해서 경쟁